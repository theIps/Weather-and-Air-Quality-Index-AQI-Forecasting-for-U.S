...
test = spark.read.csv("/home/ldua/Desktop/FinalBig/test.csv", header=True, schema=schema_test)
    ...
    for i in stateval:
        ...
        if (itr == 15):
            predictions[criteria_gas].coalesce(1).write.csv(
                '/home/ldua/Desktop/FinalBig/max_value/predicted_max_value_' + str(criteria_gas) + '_' + str(j),
                sep=',',
                header=True)
            predictions[criteria_gas] = spark.createDataFrame(sc.emptyRDD(), schema=schema2)
            itr = 0
