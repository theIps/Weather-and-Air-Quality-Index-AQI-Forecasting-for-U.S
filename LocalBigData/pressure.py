...
for year in range(2013, 2018):
    support = spark.read.csv("/home/ldua/Desktop/BigDataProject/support/daily_PRESS_" + str(year) + ".csv", header=True)
...
train_final = train_final.withColumn('am_press', train_final['am_press'].cast('Integer'))
...
scheme3 = types.StructType([types.StructField('month', types.IntegerType(), True),
                            types.StructField('year', types.IntegerType(), True),
                            types.StructField('predicted_press', types.IntegerType(), True),
                            types.StructField('state_code', types.IntegerType(), True)
                            ])
testing = spark.read.csv("/home/ldua/Desktop/BigDataProject/test.csv", schema=schema2)
...
for i in stateval:
    print(i['state_code'])
    df = spark.sql('''select month,year,am_press from training where state_code=''' + str(
    ...
    prediction_Col_Name = "predicted_press"

    vecAssembler = VectorAssembler(inputCols=["month", "year"], outputCol="features")
    # lr = LinearRegression(featuresCol="features", labelCol="am_press", predictionCol=prediction_Col_Name)
    # rfr = RandomForestRegressor(featuresCol="features", labelCol="am_press", predictionCol=prediction_Col_Name)
    # dtr = DecisionTreeRegressor(featuresCol="features", labelCol="am_press", predictionCol=prediction_Col_Name)
    gbtr = GBTRegressor(featuresCol="features", labelCol="am_press", predictionCol=prediction_Col_Name)
    ...
    # evaluator = RegressionEvaluator(predictionCol=prediction_Col_Name, labelCol="am_press", metricName="mse")
...
predictions.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_press', sep=',', header=True)

df_final = train_final.union(predictions)
df_final.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_press_i', sep=',', header=True)
