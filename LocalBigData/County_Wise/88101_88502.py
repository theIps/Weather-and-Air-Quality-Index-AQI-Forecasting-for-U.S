...
schema = types.StructType([types.StructField('county_code', types.StringType(), True),
...
for j in ['88101','88502']:
    ...
    for year in range (1998, 2018):
        ...
        train = training.select('County Code', 'Date Local', 'Observation Count', 'Observation Percent', '1st Max Value', '1st Max Hour','Arithmetic Mean').where(training['State Code'] == 6)
        ...
        train_g = train.groupBy(train['County Code'],train['Month'],train['Year']).agg(functions.avg(train['Observation Count']).alias('Observation Count'),functions.avg(train['Observation Percent']).alias('Observation Percent'),
        ...
        for i in supportl:
             ...
             support_g = support_t.select(support_t['County Code'].alias('sc'),support_t['Month'].alias('m'),support_t['Year'].alias('y'),support_t['AM'])
             train_g = train_g.join(support_g,[(train_g['County Code'] == support_g['sc']) & (train_g['Year'] == support_g['y']) & (train_g['Month']== support_g['m'])
                                    ]).drop('sc','m','y').select('*').sort('County Code','Year','Month')

        train_final[itr] = train_final[itr].union(train_g)



    train_final[itr].coalesce(1).write.csv('/home/ldua/Desktop/County/' + str(j), sep=',', header=True)
