...
for i in stateval:
    print(i['state_code'])
    df = spark.sql('''select month,year,am_rh from training where state_code=''' + str(
...
predictions.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_rh', sep=',', header=True)

df_final = train_final.union(predictions)
df_final.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_rh_i', sep=',', header=True)
