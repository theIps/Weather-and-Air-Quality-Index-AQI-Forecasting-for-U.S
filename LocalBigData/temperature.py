...
train_final = train_final.withColumn('am_temp', train_final['am_temp'].cast('Integer'))
...
for i in stateval:
    print(i['state_code'])
    df = spark.sql('''select month,year,am_temp from training where state_code=''' + str(i["state_code"]) + ''' order by year LIMIT 60''')
...
predictions.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_temp', sep=',', header=True)

df_final = train_final.union(predictions)
df_final.coalesce(1).write.csv('/home/ldua/Desktop/Final/predicted_temp_i', sep=',', header=True)
