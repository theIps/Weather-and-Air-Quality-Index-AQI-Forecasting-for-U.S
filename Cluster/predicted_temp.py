...
def load_table(words):
   ...
   dict_data['predicted_temp'] = float(words[2])
...
for year in range(2013, 2018):
    support = spark.read.csv("support/daily_TEMP_" + str(year) + ".csv", header=True)
...
for i in stateval:
    print(i['state_code'])
    df = spark.sql('''select month,year,am_temp from training where state_code=''' + str(
